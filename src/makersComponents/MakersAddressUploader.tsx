...
interface MakersAddressUploadComponentProps {
  makersEmail: string;
}

const MakersAddressUploadComponent: React.FC<
  MakersAddressUploadComponentProps
> = ({makersEmail}) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response: AxiosResponse<any, any> = await axios.post(
        `${Config.API_URL_PAPAYATEST}/makers/address`,
        {
          email,
          zipCode,
          address1,
  ...
  return (
    <View style={styles.container}>
      <Text>makers Email: {makersEmail}</Text>
      <TextInput
...
export default MakersAddressUploadComponent;
