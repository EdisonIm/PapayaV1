...
import { getCharsPerLine } from '../../utils/textLength';
import Counter from '../../components/Counter';
import { useNavigation } from '@react-navigation/native';
import { useGetFoodItemList } from '../../hooks/item';
import withCommas from '../../utils/withCommas';

export const PAGE_NAME = 'P_ITEM_INFO';

const screenWidth = Dimensions.get('window').width;
const Pages = ({ route, navigation }: RootStackProps<"P_ITEM_INFO">) => {
  const { description, images, name, supplyPrice } = route.params;
  ...
  const componentWidth = screenWidth - 48;
  const textLength = getCharsPerLine(22, componentWidth)
  console.log(textLength, componentWidth)
  return (
    <Outer>
      <PositionBox>
      ...
      <Conatiner onScroll={onScroll}>
        ...
        />
        <ContentsTopBox>
          <Typography text='Title02SB' textColor={themeApp.colors.gray[2]}>{name}</Typography>
          <StartInfo>
            {/* <FastImage source={require("../../assets/images/star.png")} style={{ width: 24, height: 24, marginRight: 4 }} resizeMode='contain' /> */}
            <Typography text='Body05R' ellipsizeMode="tail" numberOfLines={5} maxChars={(textLength * 5) - 3} textColor={themeApp.colors.gray[4]}>{description}</Typography>
          </StartInfo>
        </ContentsTopBox>
        <HorizontalLine height={8} />
        <ContentsBox>
          <Box>
            <Typography text='Title04SB' textColor={themeApp.colors.gray[2]}>가격</Typography>
            <Typography text='Title04SB' textColor={themeApp.colors.gray[2]}>{withCommas(supplyPrice * count)}원</Typography>
          </Box>
          <Box>
            <Typography text='Title04SB' textColor={themeApp.colors.gray[2]}>수량</Typography>
            <Counter
              count={count}
              setCount={setCount}
            />
      ...
      <ButtonContainer>
        <OrderButton onPress={navigation.goBack}>
          <Typography text='BottomButtonSB' textColor={themeApp.colors.gray[0]}>주문하기</Typography>
        </OrderButton>
      </ButtonContainer>
